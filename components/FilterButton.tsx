...
export default function FilterButton({ clinics, onFilterChange }: FilterButtonProps) {
  ...
  const applyFilters = useCallback((prefectures: string[], genres: number[], ranking: string) => {
    ...
    if (ranking) {
      ...
      const sortedByRating = [...filtered].sort((a, b) => {
        if (a.star === null) return 1
        if (b.star === null) return -1
  ...
  return (
    <div className="absolute top-4 left-4 right-4 z-[1001]">
      ...
      <button
        onClick={toggleModal}
        className="w-full px-4 py-3 rounded-lg shadow-lg border border-gray-200 font-medium text-base transition-all duration-200 hover:shadow-xl hover:-translate-y-0.5 flex items-center justify-center gap-2 text-white"
        style={{
            ...
            <div className="mb-4">
              ...
              <div className="flex flex-wrap gap-2">
                {uniqueGenres.map((genreId) => (
                  <label key={genreId} className="flex items-center gap-2 cursor-pointer hover:bg-gray-50 px-3 py-2 rounded border border-gray-200 bg-white">
                    <input
                      type="checkbox"
                    ...
                    />
                    <span className="text-sm text-gray-700 font-medium">{genreMap[genreId as keyof typeof genreMap]}</span>
                    <span className="text-xs text-gray-500">
            ...
            <div className="mb-4">
              ...
              <div className="flex flex-wrap gap-2">
                {rankingOptions.map((ranking) => (
                  <label key={ranking} className="flex items-center gap-2 cursor-pointer hover:bg-gray-50 px-3 py-2 rounded border border-gray-200 bg-white">
                    <input
                      type="checkbox"
                      ...
                      className="w-4 h-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500"
                    />
                    <span className="text-sm text-gray-700 font-medium">{ranking}</span>
                  </label>
                ))}
              </div>
            </div>
